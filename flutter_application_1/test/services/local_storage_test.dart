...
void main() {
  ...
  group('LocalStorageService - Transactions', () {
    test('upsertTransaction should save and findByFingerprint should retrieve',
        () async {
      ...
      await storage.upsertTransaction(tx,
          needsUserInput: false, fingerprint: fingerprint);
    ...
    test('markConfirmed and markSynced should update status', () async {
      ...
      await storage.markConfirmed('sync_test',
          category: 'Food', type: 'EXPENSE', note: 'Updated');
    ...
    test('getUnsynced should return transactions that failed or are pending',
        () async {
      final tx = MyTransaction(
          id: 'unsynced',
          timestamp: DateTime.now(),
          ...
          isConfirmed: true);
      await storage.upsertTransaction(tx,
          needsUserInput: false, syncStatus: AppConstants.syncPending);
  ...
  group('LocalStorageService - Merchant Memory', () {
    test('saveMerchantMemory should persist and lookupMerchant should retrieve',
        () async {
      await storage.saveMerchantMemory('Starbucks', 'Food', 'EXPENSE',
          isDynamic: true);

      final memory =
          await storage.lookupMerchant('starbucks'); // Test normalization
  ...
  group('LocalStorageService - Merchant Memory (Duplicates)', () {
    test('saveMerchantMemory should not duplicate exact matches', () async {
      await storage.saveMerchantMemory('Zomato', 'Food', 'EXPENSE');
      await storage.saveMerchantMemory('Zomato', 'Food', 'EXPENSE');
  ...
  group('LocalStorageService - Migration', () {
    test('onUpgrade from v3 to v4 should add is_dynamic column', () async {
      ...
      final migrationDbName =
          'migration_v3_v4_${DateTime.now().millisecondsSinceEpoch}.db';
      ...
      // No need to instantiate LocalStorageService here, just run the SQL
      await db.execute(
